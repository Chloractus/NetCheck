...
import sys                                        #Allows for a variety of system related controls like ending the program safely
...
operating_system = sys.platform              #Gets the platform name for the operating system running the program

if operating_system == "win32":              #If it is a windows device
	count_arguement = '-n'                   #We set specific values for when we do the ping sweep
	time_measurement = 1000
	time_arguement  = '-w'

elif operating_system == "darwin":           #If it is a mac device
	count_arguement = '-c'                   #We do a different set of values for the ping sweep
	time_measurement = 1000
	time_arguement = '-W'

elif operating_system == "linux":            #Then, if it is a linux device
	count_arguement = '-c'                   #We do another different set of values
	time_measurement = 1
	time_arguement = '-w'

else:                      #If it isn't one of the main 3
	print("Unsupported operating system found. Exiting...")   #We print out a message that the operating system isn't supported
	sys.exit(1)                          #Then we hard exit because it was an error
...
def _ping(ip: str, timeout: int = 1) -> None:
	...
	try:
		...
		result = subprocess.run(                                   #Performs a subprocess and gathers the result (Success / Failure)
			['ping', count_arguement, '1',time_arguement, str(timeout * time_measurement), ip],     #This is the subprocess command that we want to run
...
def resolveNames(devices: list[dict], inSSDP: bool, DB: dict) -> None:
	"""
	...
	with ThreadPoolExecutor(max_workers=min(len(ips) * 4, 150)) as executor:        #Using the ThreadPoolExecutor, we assign a certain number of workers
		...
		oui = {mac: executor.submit(getVendor, mac, DB) for mac in macs if mac}    #We submit every MAC we found into getVendor as we as our OUI
	...
	ports = PScan(ips)                            #We submit every IPv4 we found into PScan
	...
	for device in devices:                        #We then loop through every device in our devices list
		...
		device['ports'] = ports[ip]               #Assign the ports we found for that specific device
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(                                                  #Adds another arguement to our parser
		...
		action="store_true",                                              #This arguement doesn't take in any user input, instead it returns true or false depending on if the arguement was called at all
		help="Updates/Install IEEE OUI Database"                                        #Prints out a description for this arguement when users use "-h" or "--help"
	)
